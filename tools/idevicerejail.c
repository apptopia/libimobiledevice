/*
 * idevicerun.c
 * Run app using the debugserver service of a device.
 *
...
static char loggingBuf[16384];
static int loggingBufCursor = 0;
static volatile int gShouldStop = 0;

static void syslog_callback(char c, void *user_data) {
	loggingBuf[loggingBufCursor++] = c;
	loggingBuf[loggingBufCursor] = 0;
...
static int start_logging(idevice_t dv) {
	lockdownd_client_t lockdown = NULL;
	lockdownd_error_t lerr = lockdownd_client_new_with_handshake(dv, &lockdown, "idevicesyslog");
	...
	serr = syslog_relay_client_new(dv, svc, &syslog);
...
static debugserver_error_t debugserver_client_handle_response(debugserver_client_t client, char** response, int send_reply)
{
	...
	if (r[0] == 'O') {
		...
		*response = NULL;
		...
		dres = debugserver_client_send_command(client, command, response, &response_size);
		...
		default:
			...
			while (!gShouldStop) {
				...
				if (debugserver_client) {
					...
					debugserver_client = NULL;
				}

				debugserver_client_start_service(device, &debugserver_client, "idevicerun");
				...
				if (debug_level) {
					...
					response_size = 0;
					dres = debugserver_client_send_command(debugserver_client, command, &response, &response_size);
					debugserver_command_free(command);
					command = NULL;
					if (response) {
				...
				char* packet_size[2] = {strdup("512"), NULL};
				...
				const char **app_argv = (const char **)malloc(sizeof(char*) * app_argc);
				app_argv[0] = path;
